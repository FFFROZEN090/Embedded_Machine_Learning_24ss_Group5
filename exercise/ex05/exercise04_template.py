...
class VGG11(nn.Module):
    def __init__(self, norm_type=None, num_groups=1):
        super().__init__()
    ...
    def _make_layers(self):
        layers = []
        configurations = [(3, 64), (64, 128), (128, 256, 256), (256, 512, 512), (512, 512, 512)]
        for config in configurations:
            for in_channels, out_channels in zip(config[:-1], config[1:]):
                layers.extend(self._add_conv_block(in_channels, out_channels))
    ...
    def _add_conv_block(self, in_channels, out_channels):
...
def main():
    ...
    parser.add_argument('--norm-type', type=str, default='none', choices=['none', 'batch', 'group', 'layer'], help='type of normalization')
    parser.add_argument('--num-groups', type=int, default=1, help='number of groups for GroupNorm, ignored if not GroupNorm')
